...
2. Confirm task verification passed and the task worktree is clean.
3. Discover the current/base checkout and branch with `git worktree list --porcelain`. Do not assume
   `master` or `main`; ask when the intended base is ambiguous.
...
5. On the task branch, consolidate task work relative to its merge base into exactly one business
   ...
   scope, and verifiable; otherwise abort the rebase and ask the user.
...
6. If transfer or verification fails, restore only touched paths from the external backup.

Keep the task branch and worktree after review-mode transfer so the user can inspect or recover the
source independently. Retain and report the external backup until the user accepts the review result.
...
2. Otherwise, preserve unrelated base-local changes, confirm the rebased task branch contains one
   business commit, and run `git merge --ff-only` from the base branch.
3. If fast-forward integration is refused, stop with the base and its local changes untouched; do
   not move, commit, or hide those changes to make the command succeed.
4. Re-run relevant verification and compare the preflight snapshot to prove unrelated staged,
   unstaged, and untracked state is unchanged. Remove the external backup, task worktree, and task
   branch only after verification passes and no unowned files would be removed.
