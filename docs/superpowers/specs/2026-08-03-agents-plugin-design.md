...
- 固定的规范来源；
- `request.json`；
- 带有空模型槽位的 `models.json`；
- 五个待生成输出目录；
- 每个配置第三方 Skill 的最新 `ref` 快照。

Agent 只填写模型槽位和五个语义输出。Finish 从受保护的请求推导全部执行参数，自动运行内部
apply 和 check、汇总结果并删除 session；生成无法继续时由 cancel 安全删除 session。第三方
Skill 拉取失败、名称不匹配、缺少 `SKILL.md` 或包含链接时，在目标写入前停止。
...
- 标记共享、生成和第三方 Skill 目录为整体替换根，以删除其中不在期望状态的旧文件。
