...
- 在合格的 Task Worktree 中，实施工作流可以无需单独授权，通过仓库正常 commit hook 创建仅含任务
  改动的 Checkpoint Commit。在合格的 Batch Worktree 中，每个 ticket worker 可以准确追加自己
  ticket 的 Task Commit，controller 可以通过相同 hooks 创建 batch-review Checkpoint Commits。
- implementation workflow 准备收束或交付时，应用 `finish-worktree`。由 implementation workflow
  ...
  Batch，每个 worker 负责自己的 Ticket Task Worktree 生命周期；controller 负责 Batch Worktree
  创建、whole-batch review 和 delivery，以及每项 tracker transition。target synchronization 改变
  reviewed content 时，交回 implementation workflow review，而不是交付。
