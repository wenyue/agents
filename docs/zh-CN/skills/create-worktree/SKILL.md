...
负责 worktree 的选择、创建、验证、Task Worktree 资格认定和准备交接；不负责业务实现、完成后的
改动集成或清理。
...
2. 选择一个小写、连字符分隔的任务 slug 和具名任务分支。遵循已验证的仓库分支约定；否则使用
   `worktree/<task-slug>`。所选当前宿主已创建 worktree 的复用优先于不存在性检查；需要新建时，分支
   或预期路径已经存在则停止，不得隐式附加或覆盖。
...
2. 环境准备后，运行仓库声明的基线验证。将失败基线视为既有证据，并在实施前停止，除非用户明确接受
   该基线。
3. 只有所选 worktree 使用一个具名任务分支、基线干净或已被明确接受，且每个本地路径都只属于已接受
   任务时，才将其认定为 Task Worktree。存在所有权不明确本地状态的复用 worktree 仍是 worktree，
   但不授权自主创建 Checkpoint Commit。
...
基线结果、保留的基准状态、Task Worktree 资格是否通过及其原因，以及负责后续集成或清理的所有者。
