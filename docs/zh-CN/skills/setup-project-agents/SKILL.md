...
为一个目标仓库运行由脚本支持的 setup 工作流。工作流始终启用 Codex、Cursor 和 Copilot。
Agent 为目标尚未定义的模型作出选择、创作请求的项目专属内容、审查内容并消费结构化结果；脚本
负责所有确定性 setup 操作。
...
- `generation_requests` 列出的三个 Rule 和两个 Skill 目标。把每个仓库相对 `target` 原样写到
  返回的 `generated` 目录下。例如，目标 `.agents/rules/20-project-tools.md` 必须写到
  `GENERATED/.agents/rules/20-project-tools.md`，绝不能写成
  `GENERATED/rules/20-project-tools.md`。
...
1. 使用 `start` 和目标路径调用平台 wrapper：
   ...
   把返回的 generated 路径记录为 `GENERATED`。
...
3. 把返回的 `source_root` 解析为 `SOURCE_ROOT`。读取
   ...
   把五个 `generation_requests` 目标准确写到 `GENERATED/<target>`，保留包括 `.agents/` 在内的
   每一段目标路径。
...
5. 成功 start 后，如果必须在调用 finish 前停止，只使用 `--session "$SESSION"` 调用 `cancel`。
   finish 返回后不要调用 cancel：无论成功或失败，finish 都负责清理会话。

## 停止条件

start、finish 或 cancel 发生任何错误时停止并准确报告。finish 出错后不要调用 cancel 或复用该
会话；解决报告的原因后重新 start。不要修复脚本拥有的状态、逐文件选择覆盖关系、修改请求、向
finish 传递未请求的路径或手动删除会话。

## 审查关口

- [ ] 阅读每个完整生成 Rule 和 Skill；确认它遵循创作契约并使用当前目标证据。
- [ ] 阅读填写完成的 models 文件；确认每个请求的 agent/平台都有非空模型。
- [ ] 确认请求未改变，并且 `GENERATED` 准确包含五个请求目标路径及其 `.agents/` 前缀，且没有
      未声明目录。
...
报告 finish 返回的字段：固定来源 commit、已启用平台、变更路径、第三方 Skill、保留的项目自有路径
